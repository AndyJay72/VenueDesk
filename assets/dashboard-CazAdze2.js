import{a as o,r as g,i as u}from"./sidebar-CLg1KTqr.js";import{a as p}from"./index-DBnBgkrA.js";function l(t,a="£"){const i=parseFloat(t)||0;return a+i.toLocaleString("en-GB",{minimumFractionDigits:2,maximumFractionDigits:2})}function c(t){return t?new Date(t).toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"}):"—"}function m(t){return`<span class="badge badge-${{confirmed:"success",pending:"warning",cancelled:"danger",completed:"info",fully_paid:"success"}[t]||"muted"}">${t||"—"}</span>`}const y={async mount(t){t.innerHTML=`
      ${g("/dashboard")}
      <main class="content" id="mainContent">
        <div class="page-header">
          <h1><i class="fa-solid fa-gauge-high" style="color:var(--primary);"></i> Dashboard</h1>
          <p>Welcome back, ${p.getUserName()}</p>
        </div>

        <!-- KPI row -->
        <div class="metrics-grid" id="kpiRow">
          ${[{cls:"card-pre",id:"pending",icon:"fa-clock"},{cls:"card-booked",id:"revenue",icon:"fa-sterling-sign"},{cls:"card-post",id:"contacted",icon:"fa-comment-dots"},{cls:"card-danger",id:"outstanding",icon:"fa-circle-exclamation"}].map(a=>`
            <div class="metric-card ${a.cls}" id="kpi-${a.id}" style="--pct:0%">
              <div class="dial-ring"><i class="fa-solid ${a.icon}" style="font-size:1.2rem;"></i></div>
    ...
    `,u(),this._loadAll()},async _loadAll(){await Promise.allSettled([this._loadKpis(),this._loadUpcoming(),this._loadOutstanding(),this._loadPending()])},async _loadKpis(){try{const t=await o.get("/dashboard/metrics"),a=t.pending_requests??0,i=t.contacted_today??0,e=parseFloat(t.total_revenue_month)||0,n=parseFloat(t.outstanding)||0,r=(s,d)=>`${Math.min(Math.round(s/d*100),100)}%`;[{id:"pending",label:"Pending Requests",dial:a,val:String(a),sub:"Awaiting response",pct:r(a,20)},{id:"revenue",label:"Revenue (Month)",dial:'<i class="fa-solid fa-sterling-sign" style="font-size:1.1rem;"></i>',val:l(e),sub:"This calendar month",pct:e>0?"65%":"0%"},{id:"contacted",label:"Contacted Today",dial:i,val:String(i),sub:"Follow-ups done today",pct:r(i,10)},{id:"outstanding",label:"Outstanding Balance",dial:'<i class="fa-solid fa-circle-exclamation" style="font-size:1.1rem;"></i>',val:l(n),sub:"Across all bookings",pct:n>0?"70%":"0%"}].forEach(s=>{const d=document.getElementById(`kpi-${s.id}`);d&&(d.style.setProperty("--pct",s.pct),d.innerHTML=`
      ...
      `}catch{t.innerHTML='<p style="color:var(--text-muted);font-size:0.85rem;">Failed to load pending requests.</p>'}}};export{y as default};
