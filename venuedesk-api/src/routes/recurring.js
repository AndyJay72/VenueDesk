...
async function recurringRoutes(fastify) {
  ...
  // ─── POST /recurring/check-clashes ────────────────────────────────────────
  // Checks a set of candidate dates against existing confirmed bookings for a
  // given room and time window. Returns only the dates that clash.
  // Called by the recurring-bookings frontend before submitting a new series.
  //
  // Body:
  ...
  //
  // Returns: { success, data: { clashed_dates: string[] } }
  fastify.post('/check-clashes', {
    preHandler: [fastify.authenticate],
    schema: {
      body: {
        type: 'object',
        required: ['dates', 'room_id'],
        properties: {
          dates:      { type: 'string' },
    ...
    const { rows: [row] } = await withTenantContext(tenantId, (client) =>
      client.query(
        `SELECT COALESCE(
         ...
         WHERE $1 <> ''
           AND EXISTS (
             ...
             WHERE  cb.room_id    = $2::uuid
               ...
               AND  cb.end_time   > ($5::time - interval '60 minutes')
           )`,
        [dates, room_id, tenantId, start_time, end_time]
      )
    );

    return { success: true, data: { clashed_dates: row?.clashed_dates ?? [] } };
  });


  // ─── GET /recurring/series ────────────────────────────────────────────────
  // All recurring_series for tenant with customer/room/booking/payment context.
  // Replaces: DB: Get Recurring Series (GetRecurringBookings.json)
  fastify.get('/series', {
    preHandler: [fastify.authenticate],
  }, async (request) => {
    const tenantId = request.user.tenant_id;
    const { rows } = await withTenantContext(tenantId, (client) =>
      client.query(
        `SELECT
          ...
          rs.frequency,
          CASE rs.frequency
            WHEN 'fortnightly' THEN rs.cycle_amount / NULLIF(2, 0)
            WHEN 'monthly'     THEN rs.cycle_amount / NULLIF(1, 0)
            ELSE                    rs.cycle_amount / NULLIF(4, 0)
          END                                          AS rate_per_session,
          COALESCE(NULLIF(rs.series_name,''), 'Series') AS series_reference,
          rs.cycle_amount                              AS monthly_fee,
          rs.cycle_amount                              AS agreed_price,
          rs.cycle_amount                              AS monthly_amount_due,
          CASE rs.frequency
            WHEN 'fortnightly' THEN 2
            WHEN 'monthly'     THEN 1
            ELSE                    4
          END                                          AS sessions_per_cycle,
          NULL::int                                    AS billing_day,
          NULL::int                                    AS total_cycles,
          NULL::int                                    AS remaining_cycles,
          rs.active,
          rs.end_date::text,
          rs.start_date::text,
          rs.created_at::text,
          ...
          COUNT(cb.id) FILTER (WHERE cb.status = 'pending')    AS pending_count,
          COUNT(cb.id) FILTER (WHERE cb.status = 'confirmed')  AS confirmed_count,
          COUNT(cb.id) FILTER (WHERE cb.status = 'cancelled')  AS cancelled_count,
          MIN(cb.booking_date)::text                            AS first_date,
          MAX(cb.booking_date)::text                            AS last_date,
          MIN(cb.booking_date) FILTER (
  ...
  // ─── GET /recurring/outstanding-payments ─────────────────────────────────
  // Pending outstanding_payments with customer/room/rule context.
  // Replaces: DB: Fetch Outstanding Payments (GetOutstandingPayments.json)
  fastify.get('/outstanding-payments', {
    preHandler: [fastify.authenticate],
  }, async (request) => {
    const tenantId = request.user.tenant_id;
    const { rows } = await withTenantContext(tenantId, (client) =>
      client.query(
        ...
        FROM bookings.outstanding_payments op
        JOIN bookings.customers       c   ON c.id  = op.customer_id
        JOIN bookings.recurring_rules rr  ON rr.id = op.recurring_rule_id
  ...
  // ─── POST /recurring/update-rule ─────────────────────────────────────────
  // Update a recurring_rules row and log audit interaction.
  // Body: { rule_id, room_id?, day_of_week?, start_time?, end_time?,
  //         frequency?, end_date?, rate_per_session?, active?,
  //         customer_id, customer_name, customer_email, staff_member }
  // Replaces: DB: Update Rule + DB: Log Audit (UpdateRecurringRule.json)
  fastify.post('/update-rule', {
    preHandler: [fastify.authenticate],
    schema: {
      body: {
        type: 'object',
    ...
    return withTenantContext(tenantId, async (client) => {
      const { rows, rowCount } = await client.query(
        `UPDATE bookings.recurring_rules
         SET
           room_id          = COALESCE(NULLIF($1,'')::uuid, room_id),
           day_of_week      = CASE WHEN $2::text <> '' THEN $2::int ELSE day_of_week END,
           start_time       = CASE WHEN $3::text <> '' THEN $3::time ELSE start_time END,
           end_time         = CASE WHEN $4::text <> '' THEN $4::time ELSE end_time END,
           frequency        = COALESCE(NULLIF($5,''), frequency),
           end_date         = CASE WHEN $6::text <> '' THEN $6::date ELSE end_date END,
           rate_per_session = CASE WHEN $7::text <> '' THEN $7::numeric ELSE rate_per_session END,
           active           = $8::boolean
         WHERE id = $9::uuid
           AND tenant_id = $10
         RETURNING
           id AS rule_id, room_id::text, day_of_week, start_time::text,
           end_time::text, frequency, end_date::text, rate_per_session, active`,
        [room_id, String(day_of_week), start_time, end_time,
         frequency, end_date, String(rate_per_session), active, rule_id, tenantId]
      );

      if (rowCount === 0) {
        return { success: false, code: 'NOT_FOUND', message: `Rule ${rule_id} not found` };
      }

      const r = rows[0];
      const auditNote = `Rule ID: ${rule_id}, Day: ${r.day_of_week}, Time: ${r.start_time}–${r.end_time}, Freq: ${r.frequency}`;

      await client.query(
        `INSERT INTO bookings.customer_interactions
           (customer_id, customer_name, customer_email, customer_phone,
            subject, interaction_type, notes, timestamp, staff_member, tenant_id)
         VALUES ($1, $2, $3, '', 'Recurring rule updated', 'membership', $4, NOW(), $5, $6)`,
        [customer_id, customer_name, customer_email, auditNote, staff_member, tenantId]
      );
  ...
  // ─── POST /recurring/payment-override ────────────────────────────────────
  // Mark a recurring_payment_schedule entry as overridden and log it.
  // Body: { schedule_id, override_by, override_note, customer_id,
  //         period_start, period_end }
  // Replaces: DB: Apply Override + DB: Log Override (RecurringPaymentOverride.json)
  fastify.post('/payment-override', {
    preHandler: [fastify.authenticate],
    schema: {
      body: {
        type: 'object',
        required: ['schedule_id', 'override_by'],
        properties: {
          schedule_id:   { type: 'string' },
    ...
    return withTenantContext(tenantId, async (client) => {
      const { rows, rowCount } = await client.query(
        `UPDATE bookings.recurring_payment_schedule
         SET status = 'overridden', override_by = $2, override_note = $3
         WHERE id = $1::uuid
           AND tenant_id = $4
           AND status = 'pending'
         RETURNING id, recurring_rule_id, customer_id,
                   period_start::text, period_end::text, amount_due, status`,
        [schedule_id, override_by, override_note, tenantId]
      );

      if (rowCount === 0) {
        return { success: false, code: 'NOT_FOUND',
          message: `Schedule ${schedule_id} not found or not pending` };
      }

      const note = `Payment schedule entry overridden by ${override_by}. Period: ${period_start} – ${period_end}. Note: ${override_note}`;

      await client.query(
        `INSERT INTO bookings.customer_interactions
           (customer_id, subject, interaction_type, notes, timestamp, staff_member, tenant_id)
         VALUES ($1, 'Recurring payment overridden', 'recurring_booking', $2, NOW(), $3, $4)`,
        [customer_id, note, override_by, tenantId]
      );
  ...
  // ─── GET /recurring/active-membership-rules ──────────────────────────────
  // Replaces: DB: Get Active Rules (RecurringBookingGenerator.json)
  // Returns recurring_rules that have a membership_id and are active.
  // Safe-failure design: if bookings.memberships does not exist, returns empty
  // array rather than crashing (table is part of a deferred membership module).
  //
  // Returns: { data: [...rows] }
  fastify.get('/active-membership-rules', {
    preHandler: [fastify.authenticate],
  }, async (request) => {
    const { isService, tenant_id: tenantId } = request.user;

    // Check if memberships table exists; if not, return empty gracefully.
    const { rows: tableCheck } = await (isService
      ? withServiceContext((client) => client.query(
          `SELECT 1 FROM information_schema.tables
           WHERE table_schema = 'bookings' AND table_name = 'memberships' LIMIT 1`
        ))
      : withTenantContext(tenantId, (client) => client.query(
          `SELECT 1 FROM information_schema.tables
           WHERE table_schema = 'bookings' AND table_name = 'memberships' LIMIT 1`
        ))
    );
    ...
    if (!tableCheck.length) {
      return { success: true, data: [], count: 0, note: 'memberships table not yet migrated' };
    }
    ...
    const query = `
      ...
      JOIN bookings.memberships       m ON m.id = rr.membership_id
      JOIN bookings.customers         c ON c.id = m.customer_id
      JOIN bookings.rooms             r ON r.id = rr.room_id
      WHERE rr.active = TRUE
        AND rr.membership_id IS NOT NULL
        ${isService ? '' : 'AND rr.tenant_id = $1'}
      ORDER BY rr.tenant_id, m.plan_name
    `;

    const fn = isService
      ? (client) => client.query(query)
      : (client) => client.query(query, [tenantId]);

    const { rows } = isService
      ? await withServiceContext(fn)
      : await withTenantContext(tenantId, fn);
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // PHASE 4B — DIRTY-TEN MIGRATION ENDPOINTS
  // These replace all Postgres nodes in:
  //   BillingCycleTrigger, RecordRecurringPayment, RecurringAutoCancel
  // ═══════════════════════════════════════════════════════════════════════════


  // ─── GET /recurring/due-billing-cycles ───────────────────────────────────
  // Replaces: DB: Find Due Billing Cycles (BillingCycleTrigger.json)
  // Returns all recurring_payment_schedule rows whose billing_day matches today
  // and still have remaining_cycles > 0 and are not cancelled.
  // Service-JWT compatible: if role=service, queries all tenants; otherwise
  // restricts to the JWT tenant_id.
  //
  // Returns: { data: [...rows] }
  fastify.get('/due-billing-cycles', {
    preHandler: [fastify.authenticate],
  }, async (request) => {
    const { isService, tenant_id: tenantId } = request.user;
  ...
  // ─── POST /recurring/create-billing-record ────────────────────────────────
  // Replaces: DB: Create Outstanding Payment + DB: Decrement Remaining Cycles
  // (BillingCycleTrigger.json)
  //
  // Atomically:
  //   1. Inserts a row into outstanding_payments (idempotent ON CONFLICT DO NOTHING)
  //   2. Decrements remaining_cycles and advances period dates on the schedule row
  //
  // Body:
  //   schedule_id         UUID   required — recurring_payment_schedule.id
  //   recurring_rule_id   UUID   required
  //   customer_id         UUID   required
  //   period_end          string required — ISO date (used to compute next period_start)
  //   amount_due          number required
  //   billing_day         number required — day-of-month for due_date
  //   total_cycles        number optional
  //   cycle_number        number optional
  //   jwt                 string optional — body-tunnel
  fastify.post('/create-billing-record', {
    preHandler: [fastify.authenticate],
    schema: {
      body: {
        type: 'object',
        required: ['schedule_id', 'recurring_rule_id', 'customer_id', 'period_end', 'amount_due', 'billing_day'],
        properties: {
          schedule_id:       { type: 'string' },
          recurring_rule_id: { type: 'string' },
          customer_id:       { type: 'string' },
          period_end:        { type: 'string' },
          amount_due:        { type: ['number', 'string'] },
          billing_day:       { type: ['number', 'string'] },
          total_cycles:      { type: ['number', 'string', 'null'] },
          cycle_number:      { type: ['number', 'string', 'null'] },
          jwt:               { type: 'string' },
        },
      },
    },
    ...
    assertUUID(schedule_id,       'schedule_id');
    assertUUID(recurring_rule_id, 'recurring_rule_id');
    assertUUID(customer_id,       'customer_id');

    const amtStr    = String(parseFloat(amount_due) || 0);
    const dayStr    = String(parseInt(billing_day)  || 1);
    const totalStr  = total_cycles  != null ? String(parseInt(total_cycles)) : null;
    const cycleStr  = cycle_number  != null ? String(parseInt(cycle_number)) : null;

    return withTenantContext(tenantId, async (client) => {
      // Step 1 — Insert outstanding payment row (idempotent)
      const { rows: [op] } = await client.query(
        `INSERT INTO bookings.outstanding_payments (
           tenant_id, recurring_rule_id, customer_id,
           period_start, amount_due, due_date, status, cycle_number, total_cycles
         )
         VALUES (
           $1, $2::uuid, $3::uuid,
           (DATE_TRUNC('month', $4::date) + INTERVAL '1 month')::date,
           $5::numeric,
           DATE_TRUNC('month', $4::date + INTERVAL '1 month')
             + ($6::int - 1) * INTERVAL '1 day',
           'pending',
           NULLIF($7, ''),
           NULLIF($8, '')
         )
         ON CONFLICT (recurring_rule_id, period_start)
           WHERE status != 'cancelled'
         DO NOTHING
         RETURNING id::text, period_start::text, due_date::text`,
        [tenantId, recurring_rule_id, customer_id,
         period_end, amtStr, dayStr, cycleStr, totalStr]
      );

      // Step 2 — Decrement remaining_cycles + advance period window
      const { rows: [sched] } = await client.query(
        `UPDATE bookings.recurring_payment_schedule
         ...
         RETURNING id::text, remaining_cycles, period_start::text, period_end::text`,
        [schedule_id, tenantId]
      );
  ...
  // ─── POST /recurring/mark-billing-email-sent ──────────────────────────────
  // Replaces: DB: Mark Email Sent + DB: Log Reminder Sent (BillingCycleTrigger.json)
  //
  // Body:
  //   recurring_rule_id   UUID   required
  ...
  //   jwt                 string optional
  fastify.post('/mark-billing-email-sent', {
    preHandler: [fastify.authenticate],
    schema: {
      body: {
        type: 'object',
        required: ['recurring_rule_id', 'period_start', 'customer_id'],
        properties: {
          recurring_rule_id: { type: 'string' },
          period_start:      { type: 'string' },
          customer_id:       { type: 'string' },
          customer_email:    { type: 'string', default: '' },
          amount_due:        { type: ['number', 'string'], default: 0 },
          due_date:          { type: 'string', default: '' },
          email_subject:     { type: 'string', default: 'Payment reminder' },
          jwt:               { type: 'string' },
        },
      },
    },
    ...
    return withTenantContext(tenantId, async (client) => {
      // Step 1 — Stamp email_sent_at on outstanding_payments row
      const { rows: [op] } = await client.query(
        `UPDATE bookings.outstanding_payments
         ...
         WHERE recurring_rule_id = $1::uuid
           ...
           AND tenant_id = $3
         RETURNING id::text`,
        [recurring_rule_id, period_start, tenantId]
      );

      // Step 2 — Log reminder sent to customer_interactions
      await client.query(
        `INSERT INTO bookings.customer_interactions
           (customer_id, customer_email, subject, interaction_type, notes,
            timestamp, staff_member, tenant_id)
         VALUES
           ($1::uuid, $2, $3, 'payment_reminder',
            'Automated billing cycle reminder sent. Amount due: £' || $4 || ' by ' || $5,
            NOW(), 'System', $6)`,
        [customer_id, customer_email, email_subject, amtStr, due_date || period_start, tenantId]
      );

      return { success: true, data: { outstanding_payment_id: op?.id || null } };
    });
  });


  // ─── GET /recurring/overdue-payment-schedules ─────────────────────────────
  // Replaces: DB: Get Overdue Series (RecurringAutoCancel.json)
  // Returns recurring_payment_schedule rows that are pending + past due_date
  // with an active recurring rule.
  // Service-JWT compatible (queries across all tenants when role=service).
  //
  // Returns: { data: [...rows] }
  fastify.get('/overdue-payment-schedules', {
    preHandler: [fastify.authenticate],
  }, async (request) => {
    const { isService, tenant_id: tenantId } = request.user;
    ...
    const query = `
      SELECT
        ...
        r.name                    AS room_name,
        rr.frequency
      FROM bookings.recurring_payment_schedule ps
      JOIN bookings.customers      c   ON c.id  = ps.customer_id
      JOIN bookings.recurring_rules rr ON rr.id = ps.recurring_rule_id
      JOIN bookings.rooms           r  ON r.id  = rr.room_id
      WHERE ps.status   = 'pending'
        AND ps.due_date < CURRENT_DATE
        AND rr.active   = TRUE
        ${isService ? '' : 'AND ps.tenant_id = $1'}
      ORDER BY ps.tenant_id, ps.due_date
    `;
  ...
  // ─── POST /recurring/cancel-overdue-series ────────────────────────────────
  // Replaces: DB: Cancel Future Bookings + DB: Deactivate Rule +
  //           DB: Mark Schedule Cancelled + DB: Log Cancellation
  //           (RecurringAutoCancel.json)
  //
  // Atomically:
  //   1. Cancels all future confirmed_bookings for the rule (booking_date >= TODAY)
  //   2. Deactivates the recurring_rule
  //   3. Marks the recurring_payment_schedule entry as cancelled
  //   4. Logs cancellation to customer_interactions
  //
  // Body:
  //   recurring_rule_id  UUID   required
  //   schedule_id        UUID   required — recurring_payment_schedule.id to mark cancelled
  //   customer_id        UUID   required
  //   customer_email     string optional
  //   amount_due         number optional — for the log note
  //   due_date           string optional — for the log note
  //   jwt                string optional
  fastify.post('/cancel-overdue-series', {
    preHandler: [fastify.authenticate],
    schema: {
      body: {
        type: 'object',
        required: ['recurring_rule_id', 'schedule_id', 'customer_id'],
        properties: {
          recurring_rule_id: { type: 'string' },
          schedule_id:       { type: 'string' },
          customer_id:       { type: 'string' },
    ...
    return withTenantContext(tenantId, async (client) => {
      // Step 1 — Cancel all future bookings for this rule
      const { rows: cancelledBookings } = await client.query(
        `UPDATE bookings.confirmed_bookings
         ...
         RETURNING id::text, booking_date::text`,
        [recurring_rule_id, tenantId]
      );

      // Step 2 — Deactivate the recurring rule
      const { rows: [rule] } = await client.query(
        `UPDATE bookings.recurring_rules
      ...
      // Step 3 — Mark schedule entry as cancelled
      const { rows: [sched] } = await client.query(
        `UPDATE bookings.recurring_payment_schedule
         SET status = 'cancelled'
         WHERE id = $1::uuid AND tenant_id = $2
      ...
      await client.query(
        `INSERT INTO bookings.customer_interactions
         ...
         VALUES
           ($1::uuid, $2,
            'Recurring series auto-cancelled (non-payment)',
            'recurring_booking',
            'Recurring series automatically cancelled. Payment of £' || $3 ||
              ' was overdue (due: ' || $4 || '). Rule deactivated, ' ||
              $5 || ' future sessions cancelled.',
            NOW(), 'System', $6)`,
        [customer_id, customer_email, amtStr, due_date || 'unknown',
         cancelledBookings.length, tenantId]
      );

      await logger.info(
        'RecurringRoute',
        `Overdue series cancelled: rule=${recurring_rule_id}`,
        {
          recurring_rule_id,
          schedule_id,
          sessions_cancelled: cancelledBookings.length,
          tenant_id: tenantId,
        },
        tenantId
      );
      ...
      return {
        success: true,
        data: {
          rule_deactivated:   !!(rule),
          schedule_cancelled: !!(sched),
          sessions_cancelled: cancelledBookings.length,
          booking_ids:        cancelledBookings.map(r => r.id),
        },
      };
    });
  });
  ...
  // Replaces ALL 6 Postgres nodes in RecordRecurringPayment.json.
  // Consolidates: DB: Resolve Rule ID + DB: Mark Period Paid +
  //               DB: Record Payment Entry + DB: Log Payment +
  //               DB: Write Audit Log + DB: Apply to Sessions
  //
  // Body (matches what Code: Validate & Prepare in the workflow outputs):
  //   rule_id              UUID   required
  //   amount_paid          number required
  //   payment_method       string default 'cash'
  //   payment_plan         string default 'per_session'
  //   performed_by         string default 'Staff'
  //   customer_id          UUID   optional — enriched from DB if absent
  //   series_reference     string optional
  //   payment_schedule_id  UUID   optional — marks specific schedule row paid
  //   apply_to_sessions    bool   default false
  //   sessions_to_pay      number default 0
  //   session_price        number default 0
  //   jwt                  string optional
  fastify.post('/record-series-payment', {
    preHandler: [fastify.authenticate],
    schema: {
      body: {
        type: 'object',
        required: ['rule_id', 'amount_paid'],
        properties: {
          rule_id:             { type: 'string' },
          amount_paid:         { type: ['number', 'string'] },
          payment_method:      { type: 'string', default: 'cash' },
          payment_plan:        { type: 'string', default: 'per_session' },
          performed_by:        { type: 'string', default: 'Staff' },
          ...
          room_name:           { type: 'string', default: '' },
          series_reference:    { type: 'string', default: '' },
          payment_schedule_id: { type: 'string', default: '' },
          apply_to_sessions:   { type: 'boolean', default: false },
          sessions_to_pay:     { type: ['number', 'string'], default: 0 },
          session_price:       { type: ['number', 'string'], default: 0 },
          jwt:                 { type: 'string' },
        },
      },
    },
    ...
    const {
      ...
      sessions_to_pay:    rawSessionsToPay = 0,
      session_price:      rawSessionPrice  = 0,
    } = request.body;

    assertUUID(rule_id, 'rule_id');

    const amountPaid   = parseFloat(rawAmount)       || 0;
    const sessionsToPay = apply_to_sessions ? (parseInt(rawSessionsToPay) || 0) : 0;
    const sessionPrice  = parseFloat(rawSessionPrice) || 0;

    if (amountPaid < 0) throw unprocessable('amount_paid must be >= 0');

    return withTenantContext(tenantId, async (client) => {
      // ── Step 1: Resolve rule / series + enrich customer details ────────────
      // Supports both new-arch (recurring_series) and legacy (recurring_rules).
      const { rows: [resolved] } = await client.query(
        `SELECT
           COALESCE(rs.id, rr.id)::text                    AS series_id,
           COALESCE(rs.customer_id, rr.customer_id)::text  AS customer_id,
           COALESCE(NULLIF(rs.series_name,''), rr.series_reference, 'Contract')
                                                           AS series_reference,
           r.name                                          AS room_name,
           c.full_name                                     AS customer_name,
           c.email                                         AS customer_email,
           c.phone                                         AS customer_phone
         FROM (VALUES (NULLIF($1,'')::uuid, $2::integer)) v(p_rule_id, p_tenant_id)
         LEFT JOIN bookings.recurring_series rs
                ON rs.id = v.p_rule_id AND rs.tenant_id = v.p_tenant_id
         LEFT JOIN bookings.recurring_rules rr
                ON rr.id = v.p_rule_id AND rr.tenant_id = v.p_tenant_id
         LEFT JOIN bookings.rooms r ON r.id = COALESCE(rs.room_id, rr.room_id)
         LEFT JOIN bookings.customers c
                ON c.id = COALESCE(rs.customer_id, rr.customer_id)
         WHERE (rs.id IS NOT NULL OR rr.id IS NOT NULL)
         LIMIT 1`,
        [rule_id, tenantId]
      );

      if (!resolved) {
        throw notFound('RecurringRule/Series', rule_id);
      }

      // Prefer body values; fall back to DB-resolved values
      const effectiveCustomerId   = rawCustomerId || resolved.customer_id;
      const effectiveSeriesRef    = series_reference || resolved.series_reference;
      const effectiveCustomerName = customer_name  || resolved.customer_name  || '';
      const effectiveCustomerEmail= customer_email || resolved.customer_email || '';
      const effectiveCustomerPhone= customer_phone || resolved.customer_phone || '';
      const effectiveRoomName     = room_name      || resolved.room_name      || '';

      // ── Step 2: Mark period paid (dual-arch) ───────────────────────────────
      // Step A: reduce balance_due on recurring_series (no-op for legacy)
      await client.query(
        `UPDATE bookings.recurring_series
         SET    balance_due = GREATEST(0, balance_due - $3::numeric),
                updated_at  = NOW()
         WHERE  id = NULLIF($1,'')::uuid
           AND  tenant_id = $2::integer
           AND  $3::numeric > 0`,
        [rule_id, tenantId, String(amountPaid)]
      );

      // Step B: mark legacy schedule row paid (no-op if absent)
      await client.query('SAVEPOINT sp_schedule_mark');
      await client.query(
        `UPDATE bookings.recurring_payment_schedule
         SET    status   = 'paid',
                paid_at  = NOW()
         WHERE  id = NULLIF($1,'')::uuid
           ...
           AND  status = 'pending'
           AND  $3::numeric > 0`,
        [payment_schedule_id, tenantId, String(amountPaid)]
      ).catch(async () => {
        await client.query('ROLLBACK TO SAVEPOINT sp_schedule_mark').catch(() => {});
      });
      await client.query('RELEASE SAVEPOINT sp_schedule_mark').catch(() => {});

      // ── Step 3: Record payment entry ───────────────────────────────────────
      // Pattern 3: reference built in JS to avoid $N type conflicts
      const refBase  = `REC-${(effectiveSeriesRef || 'SER').trim()}-PAY`;
      const amtStr   = String(amountPaid);
      ...
      const { rows: [payment] } = await client.query(
        `INSERT INTO bookings.payments (
         ...
         SELECT
           ...
           $3,
           'completed',
           $4 || '-' || TO_CHAR(NOW(), 'YYYYMMDD-HH24MI') || '-' || LEFT(gen_random_uuid()::text, 8),
           NULLIF($5,''),
           $6::integer,
           NOW()
         WHERE $2::numeric > 0
         RETURNING id::text, reference_number`,
        [effectiveCustomerId, amtStr, payment_method, refBase, effectiveSeriesRef, tenantId]
      );

      // ── Step 4: Log payment in customer_interactions ───────────────────────
      const validPlans = ['in_advance','per_session','in_arrears','per_period'];
      const safePlan   = validPlans.includes(payment_plan) ? payment_plan : 'per_session';
      const noteAmt    = amountPaid > 0
        ? `. Amount paid: £${amountPaid} via ${payment_method}${payment?.reference_number ? ' · Ref: ' + payment.reference_number : ''}`
        : '. Payment deferred.';

      await client.query(
        `INSERT INTO bookings.customer_interactions
           (customer_id, customer_name, customer_email, customer_phone,
            room_name, subject, interaction_type, notes, timestamp, staff_member, tenant_id)
         VALUES
           (NULLIF($1,'')::uuid, NULLIF($2,''), NULLIF($3,''), NULLIF($4,''),
            NULLIF($8,''),
            CASE WHEN $5::numeric > 0 THEN 'Recurring payment recorded' ELSE 'Recurring payment plan set' END,
            'recurring_booking',
            'Payment plan: ' || $6 || $9,
            NOW(), $7, $10)`,
        [
          effectiveCustomerId, effectiveCustomerName, effectiveCustomerEmail, effectiveCustomerPhone,
          amtStr, safePlan, performed_by, effectiveRoomName, noteAmt, tenantId,
        ]
      );

      // ── Step 5: Audit log (non-fatal SAVEPOINT wrapper) ────────────────────
      await client.query('SAVEPOINT sp_audit_log');
      // Pattern 3: JSONB payload built in JS
      const auditPayload = JSON.stringify({
        type: 'series_payment', rule_id, series_reference: effectiveSeriesRef,
        amount_paid: amountPaid, payment_method, payment_plan: safePlan,
      });
      await client.query(
        `INSERT INTO bookings.audit_logs
           (tenant_id, action, entity, entity_id, payload, performed_by, created_at)
         VALUES ($1::integer, 'PAYMENT', 'recurring_series', NULLIF($2,''), $3::jsonb, NULLIF($4,''), NOW())`,
        [tenantId, rule_id, auditPayload, performed_by]
      ).catch(async () => {
        await client.query('ROLLBACK TO SAVEPOINT sp_audit_log').catch(() => {});
      });
      await client.query('RELEASE SAVEPOINT sp_audit_log').catch(() => {});

      // ── Step 6: Apply to sessions (oldest unpaid first) ────────────────────
      ...
      if (sessionsToPay > 0) {
        const { rowCount } = await client.query(
          `WITH sessions_to_mark AS (
           ...
           WHERE cb.id = stm.id AND cb.tenant_id = $2::integer`,
          [rule_id, tenantId, String(sessionsToPay)]
        );
        sessionsMarked = rowCount;
      }

      await logger.info(
        'RecurringRoute',
        `Series payment recorded: rule=${rule_id} amount=${amountPaid} sessions_marked=${sessionsMarked}`,
        { rule_id, amount_paid: amountPaid, payment_id: payment?.id, tenant_id: tenantId },
        tenantId
      );
      ...
      return {
        ...
        data: {
          ...
          series_reference: effectiveSeriesRef,
