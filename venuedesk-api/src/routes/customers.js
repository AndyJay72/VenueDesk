...
async function customersRoutes(fastify) {
  ...
  fastify.post('/upsert', {
    ...
    return withTenantContext(tenantId, async (client) => {
      ...
      const lookupKey   = email  ? 'lower(email) = lower($2) AND tenant_id = $1'
                                 : 'phone = $2 AND tenant_id = $1';
      const lookupParam = email || phone;

      const { rows: existing } = await client.query(
        `SELECT id, full_name, email, phone FROM bookings.customers
         WHERE ${lookupKey}
         LIMIT 1`,
        [tenantId, lookupParam]
      );
    ...
    const { rows } = await withTenantContext(tenantId, (client) =>
      client.query(
        `SELECT
         ...
         LEFT JOIN bookings.customers c ON c.id = ci.customer_id AND c.tenant_id = ci.tenant_id
         LEFT JOIN bookings.confirmed_bookings cb ON cb.id = ci.booking_id
