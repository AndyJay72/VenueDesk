...
async function customersRoutes(fastify) {
  ...
  fastify.post('/make-booking', {
    ...
    return withTenantContext(tenantId, async (client) => {
      ...
      // ── Record payment (skip for pending / zero-amount bookings) ──────────
      // Mirrors DB: Record Payment — skipping is an improvement over original
      // (original always inserted, even with payment_type='none' and amount=0).
      if (payment_type_clean !== 'none' && payment_amount > 0) {
        await client.query(
          `INSERT INTO bookings.payments
      ...
      // ── Close booking_request + update customer status (non-fatal) ────────
      // Mirrors DB: Close Request (continueOnFail: true in original).
      if (request_id) {
        await client.query(
          `UPDATE bookings.booking_requests
           SET    status = 'booked'
        ...
        await client.query(
          `UPDATE bookings.customers
           ...
           WHERE  id         = $2::uuid
             AND  tenant_id  = $3`,
          [booking_status, customer_id, tenantId]
        ).catch(() => { /* non-fatal */ });
      }
